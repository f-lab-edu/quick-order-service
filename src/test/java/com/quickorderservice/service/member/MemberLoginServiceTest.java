...
class MemberLoginServiceTest {
    ...
    public void loginSuccess() {
        HttpStatus httpStatus = loginService.login(MEMBER_ID, MEMBER_PASSWORD, httpSession);
    ...
    public void loginSuccessCheckId() {
        HttpStatus httpStatus = loginService.login(MEMBER_ID, MEMBER_PASSWORD, httpSession);
        Assertions.assertThat(loginService.getMemberId(httpSession)).isEqualTo(MEMBER_ID);
    }

    @Test
    @DisplayName("암호가 다르면 httpStatus.UNAUTHORIZED를 반환한다.")
    public void loginFailWithWrongPassword() {
        HttpStatus httpStatus = loginService.login(MEMBER_ID, MEMBER_WRONG_PASSWORD, httpSession);
        Assertions.assertThat(httpStatus).isEqualTo(httpStatus.UNAUTHORIZED);
    }

    @Test
    @DisplayName("존재하지 않은 아이디로 로그인시도하면 httpStatus.UNAUTHORIZED를 반환한다.")
    public void loginFailWithNoMember() {
        HttpStatus httpStatus = loginService.login(MEMBER_WRONG_ID, MEMBER_WRONG_PASSWORD, httpSession);
        Assertions.assertThat(httpStatus).isEqualTo(httpStatus.UNAUTHORIZED);
    }
}